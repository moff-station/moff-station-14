...
public sealed partial class VendingMachineKeypadMenu : FancyWindow
{
    ...
    private void BuildNumpad()
    {
        ...
        for (var i = 1; i <= 9; i++)
        {
            ...
            var key = new VendingMachineKeypadKey(number.ToString(), pitch);
            key.OnKeyPressed += () => OnNumberPressed(number);
            NumpadGrid.AddChild(key);
        }

        var clearKey = new VendingMachineKeypadKey(Loc.GetString("vending-machine-keypad-clear"), 0.7f);
        ...
        var zeroKey = new VendingMachineKeypadKey("0", MathF.Pow(2f, 12f / 12f));
        zeroKey.OnKeyPressed += () => OnNumberPressed(0);
        NumpadGrid.AddChild(zeroKey);

        var enterKey = new VendingMachineKeypadKey(Loc.GetString("vending-machine-keypad-enter"), 1.3f);
    ...
    private void BuildLetterGrid()
    {
        ...
        for (var row = 0; row < _rowCount; row++)
        {
            ...
            // letters use the same c scale, but shifted down a full octave
            var shift = _bluesScale[row % _bluesScale.Length] - 12;
            var pitch = MathF.Pow(2f, shift / 12f);

            var key = new VendingMachineKeypadKey(letter.ToString(), pitch);
            key.OnKeyPressed += () => OnLetterPressed(capturedLetter);
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        if (_bufferLetter != null || _bufferNumber != null)
        {
            ...
            {
                _audio.PlayGlobal(new SoundPathSpecifier("/Audio/Machines/button.ogg"), Filter.Local(), false, new AudioParams().WithVolume(-6f).WithPitchScale(0.6f));
    ...
    private void ShowFeedback(bool success)
    {
        ...
        if (success)
        {
            ...
            audio.PlayGlobal(new SoundPathSpecifier("/Audio/Machines/vending_jingle.ogg"), Filter.Local(), false, new AudioParams().WithVolume(-4f));
        ...
        else
        {
            ...
            audio.PlayGlobal(new SoundPathSpecifier("/Audio/Machines/buzz-two.ogg"), Filter.Local(), false, new AudioParams().WithVolume(-4f));
        }
