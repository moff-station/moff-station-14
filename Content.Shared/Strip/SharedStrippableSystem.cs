...
public abstract partial class SharedStrippableSystem : EntitySystem
{
        ...
        string slot)
    {
        ...
        if (!stealth)
        {
            _interactionSystem.DoContactInteraction(user, target, null, true, interactionParticles: false); // Moffstation - Interaction particles - switched to person so the particles pop up
